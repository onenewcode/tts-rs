...
mod tests {
    ...
    fn entry(path: &str, shape: &[usize], dtype: &str) -> WeightManifestEntry {
        WeightManifestEntry {
            path: path.to_string(),
            shape: shape.to_vec(),
    ...
    fn compare_manifests_reports_exact_match_for_identical_entries() {
        let source = manifest(vec![entry("a.weight", &[2, 3], "F32")]);
        let export = manifest(vec![entry("a.weight", &[2, 3], "F32")]);
    ...
    fn compare_manifests_reports_missing_keys_in_export() {
        let source = manifest(vec![
            entry("a.weight", &[2, 3], "F32"),
            entry("b.weight", &[3, 4], "F32"),
        ]);
        let export = manifest(vec![entry("a.weight", &[2, 3], "F32")]);
    ...
    #[test]
    fn compare_manifests_reports_shape_and_dtype_mismatches() {
        let source = manifest(vec![
            entry("shape.weight", &[2, 3], "F32"),
            entry("dtype.weight", &[2, 3], "F32"),
        ]);
        let export = manifest(vec![
            entry("shape.weight", &[3, 2], "F32"),
            entry("dtype.weight", &[2, 3], "F16"),
        ]);
        ...
        assert_eq!(report.mismatches[1].path, "shape.weight");
        assert!(report.mismatches[1].reason.contains("shape mismatch"));
    }
    ...
    fn write_json_creates_parent_directories() {
        ...
        let value = manifest(vec![entry("a.weight", &[1], "F32")]);
        ...
        assert_eq!(parsed["entries"][0]["path"], "a.weight");
    }
}
