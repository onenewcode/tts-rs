...
#[cfg(feature = "operator-profiling")]
static PROFILING_CONFIG: OnceLock<RwLock<ProfilingConfig>> = OnceLock::new();
...
pub fn configure(config: &ProfilingConfig) {
    #[cfg(feature = "operator-profiling")]
    {
        let lock = PROFILING_CONFIG.get_or_init(|| RwLock::new(ProfilingConfig::default()));
...
pub fn with_session_context<T>(session_id: usize, step_idx: usize, f: impl FnOnce() -> T) -> T {
    #[cfg(feature = "operator-profiling")]
    {
        CONTEXT.with(|slot| {
...
pub fn record_operator<T>(_name: &'static str, f: impl FnOnce() -> T) -> T {
    #[cfg(feature = "operator-profiling")]
    {
        let enabled = PROFILING_CONFIG
            .get_or_init(|| RwLock::new(ProfilingConfig::default()))
            .read()
