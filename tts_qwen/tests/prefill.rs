...
#[test]
fn custom_voice_prefill_builds_expected_single_sample_controls_and_masks() {
    ...
    let hidden_size = talker.config.talker_config.hidden_size;
...
#[test]
fn custom_voice_prefill_left_pads_batch_attention_and_positions() {
    let model_dir = common::resolve_model_dir();
    let device = Default::default();
    let talker = load_qwen3_tts_talker_for_inference::<Backend>(&model_dir, &device)
        .expect("talker should load");
    let tokenizer = Qwen3TtsTextTokenizer::from_model_dir(&model_dir).unwrap();
    let batch = CustomVoiceBatch {
        requests: vec![
            CustomVoiceRequest {
    ...
    assert_eq!(
        [batch_size, hidden],
        [2, talker.config.talker_config.hidden_size]
    );
