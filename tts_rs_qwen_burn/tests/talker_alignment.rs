...
use burn::tensor::{Int, Tensor, TensorData, DType};
use std::fs;
use serde::Deserialize;
use tts_rs_qwen_burn::{
    load_qwen3_tts_talker_for_inference, TalkerForwardInput, KeyValueCache, forward_talker_prefill,
};
...
fn test_numerical_alignment_with_python_reference() {
    ...
    let ref_json = fs::read_to_string(ref_path).expect("reference.json not found. Run `python py/generate_reference.py` first.");
    let ref_data: ReferenceData = serde_json::from_str(&ref_json).unwrap();
    
    // 2. Load Rust Model
    let model_dir = "../Qwen/Qwen3-TTS-12Hz-0.6B-CustomVoice";
    let loaded = load_qwen3_tts_talker_for_inference::<Backend>(model_dir, &device).expect("Failed to load Rust model");

    // 3. Prepare Input Tensors from JSON
    let flattened_embeds: Vec<f32> = ref_data.input.inputs_embeds.iter().flatten().flatten().cloned().collect();
    let inputs_embeds = Tensor::<Backend, 3>::from_data(
        TensorData::new(flattened_embeds, [1, 5, 1024]), 
        &device
    ).cast(DType::BF16); // Match model weights
    
    let flattened_pos: Vec<i32> = ref_data.input.position_ids.iter().flatten().flatten().cloned().collect();
    let position_ids = Tensor::<Backend, 3, Int>::from_data(
        TensorData::new(flattened_pos, [3, 1, 5]),
        &device
    );
    ...
    let actual_sum: f32 = actual_logits.clone().sum().into_data().convert::<f32>().into_vec::<f32>().unwrap()[0];
    ...
    let actual_first_5: Vec<f32> = actual_logits.flatten::<1>(0, 2).slice([0..5]).into_data().convert::<f32>().into_vec::<f32>().unwrap();
    for (i, (a, e)) in actual_first_5.iter().zip(ref_data.expected.logits.first_5.iter()).enumerate() {
        assert!((a - e).abs() < 1.0, "Value mismatch at index {}: rust={}, py={}", i, a, e);
    }
